...
| `README.md` | 已完成 | ✅ |
| `docs/` 及子目录 | 不存在 | ❌ 非核心需求 |
| `PROJECT_SPEC.md` | 不存在（使用 `PROJECT_STATUS.md`） | ✅ 替代 |
...
| 后端 | `https://task-management-dashboard-1mlg.vercel.app` | ✅ |
| vercel.json | `backend/vercel.json` | ✅ |
| README.md | 已完成 | ✅ |
| 部署方式 | Vercel 双项目（Flask + Next.js），免绑卡，SQLite `/tmp` 零持久化 | ✅ |
...
| 后端 8 API | CRUD + 导出 + 导入 + 健康检查 |
| 三列看板 | 待处理/进行中/已完成，响应式布局 |
| 任务操作 | 创建/编辑/删除/状态切换 |
| UI 状态 | loading/error/empty 全覆盖 |
| 扩展功能 | 截止日期、进度追踪、图片上传、导入导出、Toast 提示 |
| 部署 | Vercel 双项目，免绑卡，线上可访问 |

### ⚠ 规约约定但未满足（可忽略）

| 项目 | 说明 |
|------|------|
| 文件名规范 | 组件文件用 PascalCase（`TaskCard.tsx`）而非 kebab-case（`task-card.tsx`），不影响功能 |
| 后端 `.env` | 缺失（当前靠代码默认值，本地/部署均正常） |
| Prettier | 未单独配置（ESLint 已覆盖主要规范） |
